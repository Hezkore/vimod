...
function! s:plugin_settings()
	...
	if exists('g:enabled_lsp') && g:enabled_lsp == 1
		...
		call quickui#menu#install('&LSP', [
			...
			\ ["--"],
			\ ["Show Symb&ols", 'LspDocumentSymbol', "Display symbols in the current document"],
			\ ["Find &Symbols", 'LspDocumentSymbolSearch', "Search for symbols across the workspace"],
			\ ["Find &Workspace Symbols", 'LspWorkspaceSymbol', "Search for symbols across the workspace"],
			\ ["--"],
